"""Test screen detection against a saved screenshot."""
...
for anchor_set in SCREEN_ANCHORS:
    ...
    if detected or matches > 0:
        print(f"\n{anchor_set.screen.value}: {matches}/{len(anchor_set.anchors)} (need {anchor_set.min_matches}) {'DETECTED' if detected else ''}")
        for d in details:
            print(d)
