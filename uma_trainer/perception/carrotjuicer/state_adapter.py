...
from uma_trainer.knowledge.skill_catalog import BuyableSkill, SkillCatalog
...
# ---------------------------------------------------------------------------
# Chara effect lookup (chara_info.chara_effect_id_array → bot key + polarity)
# ---------------------------------------------------------------------------

_CHARA_EFFECT_LOOKUP_PATH = (
    Path(__file__).resolve().parents[3] / "data" / "chara_effect_lookup.json"
)


@lru_cache(maxsize=1)
def _load_effect_lookup() -> dict[int, tuple[str, str]]:
    """Load ``data/chara_effect_lookup.json`` once.

    Returns ``{effect_id: (bot_key, polarity)}``. ``polarity`` is one of
    ``"negative"``, ``"positive"``, ``"neutral"``. Generated by
    ``scripts/extract_effect_ids.py``.
    """
    if not _CHARA_EFFECT_LOOKUP_PATH.exists():
        return {}
    raw = json.loads(_CHARA_EFFECT_LOOKUP_PATH.read_text(encoding="utf-8"))
    ...
    for sid, entry in raw.items():
        ...
        if key and polarity:
            out[eid] = (key, polarity)
    return out


_LOGGED_UNKNOWN_EFFECTS: set[int] = set()


def _resolve_chara_effects(
    effect_ids: list[int],
) -> tuple[list[str], list[str]]:
    """Bucket ``chara_effect_id_array`` ints into negative + positive keys.

    Neutral entries (Fan Promise) are dropped — neither list contains them.
    Unknown ids are logged once at warn level and skipped.

    Returns ``(condition_keys_sorted, positive_statuses_sorted)`` with
    duplicates collapsed (e.g. effect_id 10 and 11 both map to
    ``"practice perfect"``).
    """
    if not effect_ids:
        return ([], [])
    lookup = _load_effect_lookup()
    negs: set[str] = set()
    pos: set[str] = set()
    for raw in effect_ids:
        ...
        if entry is None:
            if eid not in _LOGGED_UNKNOWN_EFFECTS:
                _LOGGED_UNKNOWN_EFFECTS.add(eid)
                logger.warning(
                    "Unknown chara_effect_id=%s (not in chara_effect_lookup.json); "
                    "regenerate via scripts/extract_effect_ids.py if master.mdb refreshed.",
                    eid,
                )
            continue
        key, polarity = entry
        if polarity == "negative":
            ...
            pos.add(key)
        # neutral -> dropped intentionally
    return (sorted(negs), sorted(pos))
...
def game_state_from_response(
    ...
    *,
    ...
    if chara is not None:
        ...
        _fill_skill_roster(gs, chara, skill_catalog)
...
def _fill_skill_roster(
    gs: GameState,
    chara: CharaInfo,
    skill_catalog: SkillCatalog | None,
) -> None:
    """Populate gs.{owned_skill_ids, disabled_skill_ids, buyable_skills}.

    Always fills owned/disabled (cheap, just typed reads). Only fills
    buyable_skills when a SkillCatalog is provided — that's the master.mdb
    join. Buyable list = preset + hint-only skills, with owned/disabled
    filtered out, and hint discount stamped on preset entries that also
    appear in skill_tips_array.
    """
    gs.owned_skill_ids = {e.skill_id for e in chara.skill_array if e.skill_id}
    gs.disabled_skill_ids = {int(s) for s in chara.disable_skill_id_array if s}

    if skill_catalog is None:
        gs.buyable_skills = []
        return

    excluded = gs.owned_skill_ids | gs.disabled_skill_ids
    preset = list(skill_catalog.preset_for_card(chara.card_id))
    by_skill_id: dict[int, BuyableSkill] = {
        s.skill_id: s for s in preset if s.skill_id not in excluded
    }
    # Index preset by (group_id, rarity) so hint stamping works even when
    # resolve_hint returns a different skill_id from the same group (e.g.
    # smallest-id tiebreak picks 200281 while the preset carries 200282).
    by_group_rarity: dict[tuple[int, int], BuyableSkill] = {
        (s.group_id, s.rarity): s for s in by_skill_id.values()
    }

    for hint in chara.skill_tips_array:
        bs = skill_catalog.resolve_hint(
            group_id=hint.group_id,
            rarity=hint.rarity,
            hint_level=hint.level,
        )
        if bs is None or bs.skill_id in excluded:
            continue
        existing = by_group_rarity.get((bs.group_id, bs.rarity))
        if existing is not None:
            # Preset wins on metadata; just stamp the hint discount.
            existing.hint_level = max(existing.hint_level, bs.hint_level)
        else:
            by_skill_id[bs.skill_id] = bs
            by_group_rarity[(bs.group_id, bs.rarity)] = bs

    gs.buyable_skills = list(by_skill_id.values())
...
def _fill_trainee(
    ...
    conditions, statuses = _resolve_chara_effects(chara.chara_effect_id_array)
    gs.condition_keys = conditions
    gs.positive_statuses = statuses
